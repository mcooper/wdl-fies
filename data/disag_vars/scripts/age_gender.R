...
age_gender.list <- lapply(age_gender.list, function(x) {
  #x <- age_gender.list$AFGr102
  if(nrow(x) < 5) {
    x <- x %>% complete(GDLcode, year = 2014:2018) %>% as.data.frame()
    x <- x[order(x$year),]
    x <- x %>% fill(colnames(x[,3:43]))
    if(is.na(x$f_0[1])) {x[1,3:43] <- x[2,3:43]}
    
    # tmp <- x[1,2:6]
    # if(x$year[1] == 2014) {
    #   if(length(unique(c(2014, 2018) %in% unique(x$year))) == 2) {
    #     x <- x %>% complete(GDLcode, year = 2014:2018) %>% as.data.frame()
    #     x <- x[order(x$year),]
    #     x[,8:43] <- apply(x[,8:43], 2, na.locf)
    #   } else{
    #     x <- x %>% complete(GDLcode, year = 2014:2018) %>% as.data.frame()
    #     x <- x[order(x$year),]
    #     x[,8:43] <- apply(x[,8:43], 2, na.approx)
    #   }
    # } else{
    #   x <- x %>% complete(GDLcode, year = 2014:2018) %>% as.data.frame()
    #   x <- x[order(x$year),]
...
age_gender <- do.call(rbind, age_gender.list)
