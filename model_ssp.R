#######################################################
# Use LASSO regression with a large stack of covariates
...
# Set up Model
vars <- names(moddat)[!names(moddat) %in% c('iso3', 'GDLCODE', 'year', 'fies.mod.rur', 'fies.sev.rur', 
...
ggplot(mapdat) + 
  ...
  labs(title='Rate of Moderate to Severe Food Insecurity in 2020',
			 subtite = 'Estimated with a large number of co-variates',
			 fill='')
...
ggplot(df %>% filter(term != '(Intercept)')) + 
	  geom_bar(aes(x=term, y=scaled), stat='identity') + 
		coord_flip() + 
		labs(title='Change in Rate of Food Insecurity with increase of 1 SD in Var\nFor 2020 LASSO Regression Model') + 
		theme_minimal()
