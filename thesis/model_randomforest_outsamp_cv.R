...
# do a K-fold cross validation, specify K
K <- 10

# generate modal validation matrix
modval <- data.frame(index = 1:K, 
...
moddat$index <- sample(rep(seq(1:K), ceiling(nrow(moddat)/K))[1:nrow(moddat)])
...
for(i in 1:K) {
  ...
  #take i as test sample and train model with i-1
  moddat_test <- subset(moddat, index == i)
  moddat_train <- subset(moddat, index != i)
...
# write.csv(modval, 'thesis/results/modval_cv.csv', row.names=F)
modval <- read.csv('thesis/results/modval_cv.csv')
...
# write.csv(modval, 'thesis/results/modval_wo_hic_cv.csv', row.names=F)
modval <- read.csv('thesis/results/modval_wo_hic_cv.csv')
...
#ggsave('thesis/mod.residuals_rf.png', width=5, height=5)
